...
import { Star, Calendar, UserSquare2, Plus, CircleAlert, ClipboardList, X, ChevronDown } from "lucide-react";
...
export function MobileSidebar({ toggleSidebar }) {
  ...
  // Handle logout
  const handleLogout = () => {
    localStorage.removeItem("userToken"); // Clear user session (modify as needed)
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center gap-3 mb-6 relative" ref={dropdownRef}>
        ...
        {dropdownOpen && (
          <div className={`absolute top-20 bg-white shadow-md rounded-md overflow-hidden w-40 text-black`}>
